...
export function exportYOLODataset(
  ...
  // Deterministically split images into train/val/test.
  // Sort to keep the split stable across exports.
  const sortedImages = [...images].sort((a, b) => a.fileName.localeCompare(b.fileName));
  const total = sortedImages.length;
  const valCount = total >= 2 ? Math.max(1, Math.floor(total * 0.1)) : 0;   // ~10% for val, at least 1 when possible
  const testCount = total >= 3 ? Math.max(1, Math.floor(total * 0.1)) : 0;  // ~10% for test, at least 1 when possible
  const trainCount = Math.max(0, total - valCount - testCount);

  const splits: Record<'train' | 'val' | 'test', ProjectImage[]> = {
    train: sortedImages.slice(0, trainCount),
    val: sortedImages.slice(trainCount, trainCount + valCount),
    test: sortedImages.slice(trainCount + valCount),
  };

  for (const split of ['train', 'val', 'test'] as const) {
    for (const image of splits[split]) {
  ...
  // If we couldn't allocate any images to val/test (very small datasets), fall back to train path for YAML
  const hasVal = splits.val.length > 0;
  const hasTest = splits.test.length > 0;

  // Generate data.yaml for YOLO training using standard folder layout
...
test: ${hasTest ? 'images/test' : hasVal ? 'images/val' : 'images/train'}
