...
import { X, Play, Square, Loader2, AlertCircle, CheckCircle, FolderOpen, Download, FileImage, Cpu, Monitor, Zap } from 'lucide-react';
...
interface YOLOTrainingDialogProps {
  onClose: () => void;
}
...
export function YOLOTrainingDialog({ onClose }: YOLOTrainingDialogProps) {
  // Project data
  ...
  return (
    <div className="yolo-training-dialog-overlay" onClick={(e) => e.target === e.currentTarget && !isTraining && onClose()}>
      <div className="yolo-training-dialog">
        <div className="dialog-header">
          <h2>YOLO Keypoint Training</h2>
          <button
            className="close-button"
            onClick={onClose}
            disabled={isTraining}
            title={isTraining ? 'Stop training first' : 'Close'}
          >
            <X size={20} />
          </button>
        </div>
          ...
          {dependencies?.installed && (
            <>
              ...
              {gpuHardware?.gpuEnabled && (
                <section className="backend-selection">
        ...
        <div className="dialog-footer">
          <button
            ...
            onClick={isTraining ? handleStopTraining : onClose}
          >
            {isTraining ? (
              <>
                <Square size={16} />
